...
export function getChunks(file, blockSize) {
  ...
  for (let i = 0; i < count; i++) {
    const chunk = file.slice(blockSize * i, i === count - 1 ? file.size : blockSize * (i + 1));
    chunks.push(chunk);
  }
  return chunks;
}
...
export function getUploadUrl(config, token) {
  ...
  return getUpHosts(token).then(res => {
...
function getUpHosts(token) {
  try {
    ...
    const url = `${getAPIProtocol()}//api.qiniu.com/v2/query?ak=${putPolicy.ak}&bucket=${
      putPolicy.bucket
    }`;
